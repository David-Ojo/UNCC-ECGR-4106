...
with open(FILE_NAME, "r", encoding="utf-8") as f:
    for line in f:
        ...
        if len(parts) >= 2:
            fra = normalize_text(parts[0])
            eng = normalize_text(parts[1])
...
fra_vocab = build_vocab([p[0] for p in train_pairs])
eng_vocab = build_vocab([p[1] for p in train_pairs])
...
class TranslationDataset(Dataset):
    def __init__(self, pairs, fra_vocab, eng_vocab):
        self.pairs = pairs
    ...
    def __getitem__(self, idx):
        fra, eng = self.pairs[idx]

        src = sentence_to_indices(fra, self.fra_vocab)
        tgt = sentence_to_indices(eng, self.eng_vocab)

        return src, tgt

train_dataset = TranslationDataset(train_pairs, fra_vocab, eng_vocab)
val_dataset = TranslationDataset(val_pairs, fra_vocab, eng_vocab)
...
def translate_sentence(model, sentence, max_len=MAX_LEN + 2):
    ...
    src = sentence_to_indices(sentence, fra_vocab).unsqueeze(0).to(device)

    generated = [eng_vocab[SOS_TOKEN]]
    ...
    with torch.no_grad():
        for _ in range(max_len):
            ...
            if next_token == eng_vocab[EOS_TOKEN]:
                break

            generated.append(next_token)

    return indices_to_sentence(generated[1:], eng_idx_to_word)
...
def compute_exact_match_and_bleu(model, val_pairs):
    ...
    for fra, eng in val_pairs:
        predicted = translate_sentence(model, fra)

        reference_tokens = eng.split()
